...
[g,p]=bode(tf(num,den),interval(1));grid on
y(1)=20*log10(g);
indice=2;
y_old=y(1);
i_old=interval(1);
...
%bodemag(sys);
[m1,m2,m3]=bode(tf(num,den),{interval(1,1),interval(1,end)});
hold on;
...
[g,p,w3]=bode(tf(num,den),interval(1));grid on
pp=p;
x=round(p/90);
p=90*x;
yf(1)=p;
...
%grid on;
[m1,m2,m3]=bode(tf(num,den),{interval(1,1),interval(1,end)});grid on
diff=m2(1)-pp;
m2=m2-diff;
hold on
